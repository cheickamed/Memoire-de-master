...
img=cv2.imread('Lena-Canny.png')
def select_recogRf(a,n,img):
    d1=time.time()
    global A
    global C
    #print(img.shape)
    ...
    for x in range(0,h-tab[0][0],tab[0][0]):
        for y in range(0,l-tab[1][0],tab[1][0]):
            #A=(x,y)
            #C=(x+tab[0][1],y+tab[1][1])
            print(compt((x,y),(x+tab[0][0],y+tab[1][0]),n,img))
            if (compt((x,y),(x+tab[0][0],y+tab[1][0]),n,img)/d)>=a:
